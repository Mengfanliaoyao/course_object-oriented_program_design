...
void TRA_switch::recv_handler (packet *p)
{
  ...
  if (p->type() == "TRA_ctrl_packet" && !is_packet_received(p)) // the switch receives a packet from the controller
  {
    ...
    if( p3->getHeader()->getSrcID() == getNodeID() ) // if this packet is sent from this node
    {
      ...
      if (networkInfoMap.find(getNodeID()) == networkInfoMap.end()) // haven't saved or need update
    ...
    else // if this packet is sent from the other node
    {
      ...
      if (networkInfoMap.find(src_id) == networkInfoMap.end()) // haven't saved or need update
        networkInfoMap[src_id] = l3->getNetwInfo() ; // deep copy

      // bfs
      if (networkInfoMap.size() == networkNodeNum) // complete
      {
        ...
        while (!q.empty())
        {
          ...
          {
            for (auto it2 = n.rbegin(); it2 != n.rend(); ++it2)
            {
              unsigned int v = it2->first;
...
//    get next hop by nextHopMap[next_label], change packet header and transit it
    ...
    {
#if BUILD_UP_ALL_LABELS
      h4->push_label(0); // dst
      h4->push_label(2); // intermediate
#else
      h4->push_label(h4->getDstID()); // only use dst as label
#endif
...
int main(void)
{
  ...
  {
    unsigned int id, src, dst, flow_size, start_time, next_broadcast_time;
