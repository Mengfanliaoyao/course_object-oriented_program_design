...
//    bool hi; // this is used for example; you can remove it when doing hw2
    ...
    unsigned int broadcast_frequncy = ONE_HOP_DELAY, total_time = 0;
...
double degree_evaluation(double degree, double c=1, double a=3, double b=1.5) // c + a*d*e^(-bd)
{
...
bool TRA_switch::buildAndPushLabels(TRA_data_packet* p4, TRA_data_header* h4)
{
  ...
  {
    double step = static_cast<double>(path.size() - 1) / static_cast<double>(labelLimit - 1);
    for (unsigned int i = 0; i < labelLimit; ++i)
    {
...
void TRA_switch::recv_handler (packet *p)
{
  ...
  if (p->type() == "TRA_ctrl_packet" && !is_packet_received(p)) // the switch receives a packet from the controller
  {
    ...
    else // if this packet is sent from the other node
    {
      ...
      // bfs
      if (networkInfoMap.size() == networkNodeNum && !nextHopMap.size()) // complete
      {
        ...
        while (!q.empty())
        {
          ...
          for (const auto& [nb_id, info] : n)
          {
            size_t degree = 0;
            auto it2 = networkInfoMap.find(nb_id);
            if (it2 != networkInfoMap.end()) degree = it2->second.size();

            neighbors.push_back({ nb_id, degree, info.first });
          }
#if 0
          std::sort(neighbors.begin(), neighbors.end(),
            [&](const NeighborKey& a, const NeighborKey& b)
            {
              
              if (a.degree != b.degree) return degree_evaluation(a.degree) > degree_evaluation(b.degree);
              return (getNodeID() % 2) ? (a.nb_id < b.nb_id) : (a.nb_id > b.nb_id);
            });
#endif
          ...
          for (const auto& [v,d,c] : neighbors)
          {
            if (visited[v])
              continue;
            ...
            if (src == u)
              ...
              nextHopMap[v] = nextHopMap[u];

            q.push(v);
          }
        }
      }
      nextHopMap.erase(getNodeID());
#if 0
...
//    get next hop by nextHopMap[next_label], change packet header and transit it
    ...
    {
      h4->pop_label(); // pop one label from the header
    }
...
int main(void)
{
  ...
  for (size_t i = 0; i < n_task; i++) // data packet event
  {
    ...
    data_packet_event(src, dst, flow_size, start_time);
  }
