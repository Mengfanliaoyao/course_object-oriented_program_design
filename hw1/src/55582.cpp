...
int main(void)
{
  ...
  std::sort(tasks.begin(), tasks.end(), [](const TaskFlow& a, const TaskFlow& b) { return a.flowSize > b.flowSize; });
  ...
  for (uint i = 0; i < n_task; i++)
  {
      ...
      std::unique_ptr<std::vector<std::pair<uint, uint>>> labels = g.widestPathDijkstra(src, dst, tasks[i].flowSize, successed);
      if (successed && labels)
      {
          while(labels->size() > label_limit)
          {
            successed = false;
  ...
  for (uint i = 0; i < n_selected; i++)
  {
      ...
      {
          std::cout << tasks[task_id].labels->at(j).first << ' ';
      }
      std::cout << '\n';
  }
  std::cout << final_flow;
