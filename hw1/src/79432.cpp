...
//  auto hop_max_comp = [&](const TaskFlow& a, const TaskFlow& b) { return g.getHops()[a.src][a.dst] < g.getHops()[a.src][a.dst]; };
//  std::sort(tasks.begin(), tasks.end(), hop_max_comp);
...
std::unique_ptr<std::vector<std::pair<uint, uint>>> Graph::minLabelWidestPath(uint src, uint dst, uint max_labels, uint task_flow, bool& success)
{
    ...
    auto base_path = widestPathDijkstra(src, dst, task_flow, base_path_ok);
