...
private:
  ...
  void bfs(uint src, ShuffleStrategy strategy = SEQUENTIAL);
private:
...
//  std::sort(tasks.begin(), tasks.end(), flow_max_comp);
...
void Graph::findAllPairShortestPath()
{
  ...
  {
      bfs(src, (ShuffleStrategy)(src % 2));
  }
}

void Graph::bfs(uint src, ShuffleStrategy strategy)
{
