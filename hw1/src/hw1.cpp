...
int main(void)
{
  ...
  // select
  std::vector<uint> selected_tasks, remaining_tasks;
  uint final_flow = 0;
...
//  auto flow_min_comp = [](const TaskFlow& a, const TaskFlow& b) { return a.flowSize < b.flowSize; };
  ...
  for (uint i = 0; i < remaining_tasks.size(); i++)
  {
      uint task_id = remaining_tasks[i];
      uint src = tasks[task_id].src, dst = tasks[task_id].dst, task_flow = tasks[task_id].flowSize;
...
std::unique_ptr<std::vector<std::pair<uint, uint>>> Graph::widestPathBfs(uint src, uint dst, uint task_flow, bool& success)
{
    ...
    // parent[lid] ”õ‰õµΩﬂ_ v µƒ«∞ÚåﬂÅE
    std::vector<uint> parent(n_node, UINT_MAX);  // -1 ±˙Êæüo∏∏
    std::vector<uint> parent_link(n_node, UINT_MAX);  // «∞ÚåﬂÅEID
...
bool Graph::goAPSP(uint src, uint dst, uint task_flow, std::vector<std::pair<uint, uint>>& path_out)
{
    // èä÷∆÷ÿΩ®£¨üoólº˛«Âø’
    path_out.clear();
    path_out.shrink_to_fit();  // ·å∑≈≈f»›¡ø
    path_out.reserve(16);      // ˚]∑÷≈ÅE
...
std::unique_ptr<std::vector<std::pair<uint, uint>>> Graph::minLabelWidestPath(uint src, uint dst, uint max_labels, uint task_flow, bool& success)
{
        ...
        for (uint labels_used = 0; labels_used <= max_labels; ++labels_used) {
            ...
            if (labels_used > 0) {
                for (int prev_pos = 0; prev_pos < pos; ++prev_pos) {
                    ...
                    // APSP ôz≤È£∫prev_node °ÅEcurr_node
    ...
    while (current_pos > 0) {
        ...
        if (current_labels != prev_labels) {  // –¬‘ÅElabel
